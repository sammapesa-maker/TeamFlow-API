...
class TeamMemberQueryParams(BaseModel):
    ...
    user_id: Optional[int] = Field(
        default=None,description="Filter by user id", ge=1
    )
    team_id: Optional[int] = Field(
        default=None, description="Filter by team id", ge=1
    )
    ...
    # Sorting Fields
    sort_by: TeamMemberSortField = Field(
        default=TeamMemberSortField.id
    )
